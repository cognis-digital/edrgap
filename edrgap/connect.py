"""Native cognis-connect emit for edrgap — forward findings to any platform.

Maps edrgap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    edrgap ... --format json | edrgap-emit --to stix
    edrgap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('host', '') + ' ' + rec.get('name', '')
